...
If you change `index.html` later, bump the `CACHE` name in `sw.js` (for example to `snakebox-v2`) so installed copies pick up the new version.
...
- Tap a snake vertex to cut the snake there or delete it. Tap an empty vertex to start a new snake when a slot is free.
